...
const Header = () => {
  ...
  return (
    <header className="bg-white shadow-lg">
      <div className="container mx-auto px-6 py-4 flex flex-col sm:flex-row justify-around items-center">
        <div className="flex items-center mb-2 sm:mb-0">
          <Link to="/" className="flex items-center">
            ...
            <span className="font-bold text-xl text-gray-800">Blogs App</span>
          </Link>
        </div>
        <SearchBar onSearch={() => {}} placeholder="Search blogs" />
        <nav className="flex flex-col sm:flex-row items-center space-x-0 sm:space-x-1">
        ...
        <div className="flex flex-col sm:flex-row space-x-0 sm:space-x-4 items-center">
          {user ? (
            <div className="relative flex items-center space-x-2 group transition-transform duration-300 hover:scale-105 z-50">
              <img src={typeof user.avatar === 'string' ? user.avatar : 'default-avatar.png'} alt="User" className="w-11 h-11 rounded-full border-2 border-gold-400 shadow-lg transition-shadow duration-300 hover:shadow-xl" />
              ...
              <div className="absolute hidden group-hover:block bg-white shadow-lg mt-[9rem] rounded-md z-50 transition-opacity duration-300 opacity-0 group-hover:opacity-100">
                <div className="flex flex-col px-4 py-3">
                  <Link to="/profile" className="flex items-center w-full py-2 text-gray-700 hover:bg-gray-100 transition-colors duration-200">
                  ...
                  <button onClick={logout} className="flex items-center w-full py-2 text-gray-700 hover:bg-gray-100 transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="inline w-5 h-5 mr-2">
